...
public class search extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 ...
		 일 경우 아이디 결과값을 출력하며, 만약 이름과 이메일 정보중 틀릴 경우 error라는 메세지를 콘솔로
		 출력합니다.
		 */
		String members[][] = {
				{"장진호","배유미","염무원","김승균","김경민","장진호","김경민"},
				{"jang_ho@nate.com","bea_mi@nate.com","number1@gmail.com",
		...
		while(w < ea) {
			...
			if(usernm==members[0][w]) { //이름 검토
				if(userem==members[1][w]) { //이메일 검토
					count = 1;	//이름과 이메일이 동일 할 경우 카운팅값을 변화시킴
					msg = members[2][w];	//해당 아이디 데이터를 문자열 변수에 삽입
					break;	//모두 일치하면 반복문 정지.
				}	
			}
			w++;
		}		
		if(count==0) {	//해당 배열에 조건이 아무것도 매칭하지 않을 경우
			msg = "error";
		}

...
public class login extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String upw = request.getParameter("upw");	//사용자가 입력패스워드(암호화X)
		...
		try {
			...
			while(rs2.next()) {	//Database에서 문자형 변수로 변환작업 하는 코드
				...
				sql_use = rs2.getString("suse").intern();	//로그인 유/무 확인
			}
			con2.close();
			ps.close();
			PrintWriter pw = response.getWriter();
			if(sql_id=="") {	//sql문법 실행시 해당 정보가 없을 경우
				pw.write("<script>alert('해당 사용자 정보를 확인 하지 못하였습니다.'); "
						+ "history.go(-1);</script>");
			}else {	//아이디가 확인 되고, 패스워드를 검토할때 사용
				if(sql_pw.equals(user_pw)) {	//동일한 패스워드일 경우
					if(sql_use == "Y") {
						pw.write("<script>alert('로그인 하셨습니다.');</script>");
						
					}else {
						pw.write("<script>alert('관리자권한으로 로그인을 막았습니다.'); hostory.go(-1)</script>");
